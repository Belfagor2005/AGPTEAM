...
class AglareAccess(Poll, Converter):
    ...
    def CamName(self):
        ...
        sername = []
# OpenPLI/SatDreamGr
        if exists("/etc/init.d/softcam") and not exists("/etc/image-version") or exists(
                "/etc/init.d/cardserver") and not exists("/etc/image-version"):
            try:
...
# OE-A
        if exists("/etc/image-version") and not exists("/etc/.emustart") and not exists(
                "/etc/CurrentBhCamName") and not exists("/etc/EGCamConf") and not exists("/etc/clist.list"):
...
# OpenVix_script
        if exists("/etc/image-version") and exists(
