...
class ServicePosition(Poll, Converter):
    ...
    def __init__(self, type):
        ...
        if type == "Length":
            ...
            raise ElementError(
                "type must be {Length|Position|Remaining|Gauge|Summary|Startendtime} with optional arguments {Negate|Plus|Detailed|ShowHours|ShowNoSeconds|7segment} for ServicePosition converter")
    ...
    def getSeek(self):
        ...
        if self.type == self.TYPE_START_END_TIME and s and (
                self.service is None or s != self.service):
            self.service = s
    ...
    def getText(self):
        ...
        if seek is None:
            ...
            if not self.detailed:
                ...
                if not self.vfd:
                    if self.showHours:
                        if self.showNoSeconds:
                            ...
                            return sign + \
                                "%d:%02d:%02d" % (l / 3600, l % 3600 / 60, l % 60)
                    else:
                        if self.showNoSeconds:
                            return sign + "%d" % (l / 60)
                        else:
                ...
                if self.showHours:
                    return sign + "%d:%02d:%02d:%03d" % (
                        (l / 3600 / 90000), (l / 90000) % 3600 / 60, (l / 90000) % 60, (l % 90000) / 90)
                else:
                    return sign + \
                        "%d:%02d:%03d" % ((l / 60 / 90000), (l / 90000) % 60, (l % 90000) / 90)
    ...
    def changed(self, what):
        cutlist_refresh = what[0] != self.CHANGED_SPECIFIC or what[1] in (
            iPlayableService.evCuesheetChanged,)
        time_refresh = what[0] == self.CHANGED_POLL or (
            what[0] == self.CHANGED_SPECIFIC and what[1] in (
                iPlayableService.evCuesheetChanged,))

        if cutlist_refresh:
            if self.type == self.TYPE_GAUGE:
                # Handle the case if downstream_elements is a list or a single
                # object
                if isinstance(self.downstream_elements, (list, tuple)):
                    for elem in self.downstream_elements:
                        if hasattr(
                                elem, "cutlist_changed") and callable(
                                elem.cutlist_changed):
                            elem.cutlist_changed()
                else:
                    if hasattr(
                            self.downstream_elements,
                            "cutlist_changed") and callable(
                            self.downstream_elements.cutlist_changed):
                        self.downstream_elements.cutlist_changed()

        if time_refresh:
            # Call changed(what) on downstream_elements similarly, checking for
            # method
            if isinstance(self.downstream_elements, (list, tuple)):
                for elem in self.downstream_elements:
