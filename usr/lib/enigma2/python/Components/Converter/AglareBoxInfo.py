                ...
                if info.startswith('ARM') and isfile('/proc/stb/info/chipset'):
                    for line in open('/proc/cpuinfo'):
                        if 'model name' in line or 'Processor' in line:
                            info = line.split(
                                ':')[-1].split()[0].strip().strip('\n')
                            info = '%s (%s)' % (open('/proc/stb/info/chipset').readline().strip().lower().replace(
                                'hi3798mv200',
                                'Hi3798MV200').replace(
                                'bcm',
                                'BCM').replace(
                ...
                if not cpu_speed:
                    try:
                        ...
                        try:
                            import binascii
                            f = open(
                                '/sys/firmware/devicetree/base/cpus/cpu@0/clock-frequency', 'rb')
                            ...
                            cpu_speed = '-'
                if cpu_info == '':
                    return _('%s, %s MHz (%d %s)') % (info, cpu_speed,
                                                      cpu_count, cpu_count > 1 and cores or core)
            else:
                return _('No info')
