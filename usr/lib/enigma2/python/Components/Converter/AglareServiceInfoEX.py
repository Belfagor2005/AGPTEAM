    ...
    for text in open("/etc/issue"):
        ...
        if 'open' not in image:
            codec_data = {-1: 'N/A',
                          ...
                          6: 'HEVC',
                          7: ' '}
            gamma_data = {-1: 'N/A', 0: 'SDR',
                          1: 'HDR', 2: 'HDR10', 3: 'HLG', 4: ' '}
        else:
            gamma_data = {-1: 'N/A', 0: 'SDR',
                          1: 'HDR', 2: 'HDR10', 3: 'HLG', 4: ' '}
            codec_data = {-1: 'N/A',
...
class AglareServiceInfoEX(Poll, Converter, object):
    ...
    def getText(self):
        ...
        if self.getServiceInfoString(
                info, iServiceInformation.sAudioPID) != "N/A":
            self.stream['apid'] = "%0.4X" % int(
                self.getServiceInfoString(
                    info,
                    iServiceInformation.sAudioPID))
        if self.getServiceInfoString(
                info, iServiceInformation.sVideoPID) != "N/A":
            self.stream['vpid'] = "%0.4X" % int(
                self.getServiceInfoString(
                    info,
                    iServiceInformation.sVideoPID))
        if self.getServiceInfoString(info, iServiceInformation.sSID) != "N/A":
            self.stream['sid'] = "%0.4X" % int(
                self.getServiceInfoString(
                    info,
                    iServiceInformation.sSID))
        if self.getServiceInfoString(info, iServiceInformation.sONID) != "N/A":
            self.stream['onid'] = "%0.4X" % int(
                self.getServiceInfoString(info, iServiceInformation.sONID))
        if self.getServiceInfoString(info, iServiceInformation.sTSID) != "N/A":
            self.stream['tsid'] = "%0.4X" % int(
                self.getServiceInfoString(info, iServiceInformation.sTSID))
        if self.getServiceInfoString(
                info, iServiceInformation.sPCRPID) != "N/A":
            self.stream['prcpid'] = "%0.4X" % int(
                self.getServiceInfoString(info, iServiceInformation.sPCRPID))
        if self.getServiceInfoString(
                info, iServiceInformation.sPMTPID) != "N/A":
            self.stream['pmtpid'] = self.getServiceInfoString(
                info, iServiceInformation.sPMTPID)
        if self.getServiceInfoString(
                info, iServiceInformation.sTXTPID) != "N/A":
            self.stream['txtpid'] = self.getServiceInfoString(
                info, iServiceInformation.sTXTPID)
        ...
        if self.getServiceInfoString(
                info, iServiceInformation.sVideoHeight) != "N/A":
            self.stream['yres'] = self.getServiceInfoString(
                info, iServiceInformation.sVideoHeight) + (
                "i", "p", "")[
                info.getInfo(
                    iServiceInformation.sProgressive)]
        if self.getServiceInfoString(
                info, iServiceInformation.sVideoWidth) != "N/A":
            self.stream['xres'] = self.getServiceInfoString(
                info, iServiceInformation.sVideoWidth)
        try:
            self.stream['gamma'] = gamma_data[info.getInfo(
        ...
        if audio:
            if audio.getCurrentTrack() > -1:
                self.stream['atype'] = str(
                    audio.getTrackInfo(
                        audio.getCurrentTrack()).getDescription()).replace(
                    ",", "")
        self.stream['vtype'] = codec_data[info.getInfo(
            iServiceInformation.sVideoType)]
        self.stream['avtype'] = self.stream['vtype'] + \
            '/' + self.stream['atype']
        if self.getServiceInfoString(
                info, iServiceInformation.sFrameRate, lambda x: "%d" %
                ((x + 500) / 1000)) != "N/A":
            self.stream['fps'] = self.getServiceInfoString(
                info, iServiceInformation.sFrameRate, lambda x: "%d" %
                ((x + 500) / 1000))
        if self.getServiceInfoString(
                info, iServiceInformation.sTransferBPS, lambda x: "%d kB/s" %
                (x / 1024)) != "N/A":
            self.stream['tbps'] = self.getServiceInfoString(
                info, iServiceInformation.sTransferBPS, lambda x: "%d kB/s" %
                (x / 1024))
        ...
        if self.type == self.apid:
            ...
            encryption_map = {
                ...
                0x900: "NDS",
                0x1800: "Nagra"}
            return encryption_map.get(
                caidinfo[0], "Unknown") if caidinfo else "FTA"
            ...
            progressive = "p" if info.getInfo(
                iServiceInformation.sProgressive) == 1 else "i"
            ...
            streaminfo = self.stream['xres'] + 'x' + self.stream['yres'] + \
                '|' + self.stream['fps'] + '|' + self.stream['gamma']
        elif self.type == self.Resolution:
            streaminfo = self.stream['xres'] + 'x' + \
                self.stream['yres'] + self.stream['fps']
    ...
    def createAudioCodec(self):
        ...
        if audio:
            try:
                ...
                if _("rus") in languages or _(
                        "Russian") in languages or _("ru") in languages:
                    languages = _("rus")
