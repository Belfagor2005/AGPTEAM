...
def clean_for_tvdb_optimized(title):
    ...
    if not isinstance(title, (str, bytes)):
        try:
            ...
            logger.error(
                f"clean_for_tvdb_optimized Title conversion error '{
                    title!r}': {
                    str(e)}")
            return ""
    ...
    try:
        ...
        # Convert to ASCII (remove accents) but keep original if conversion
        # fails
        try:
            title = normalize('NFKD', title)
            title = title.encode('ascii', 'ignore').decode('ascii')
        ...
        title = sub(
            r'^(il|lo|la|i|gli|le|un|uno|una|a|an)\s+',
            '',
            title,
            flags=IGNORECASE)
        title = sub(
            r'\s+(il|lo|la|i|gli|le|un|uno|una|a|an)$',
            '',
            title,
            flags=IGNORECASE)
...
def clean_for_tvdb(title):
    ...
    if not isinstance(title, (str, bytes)):
        try:
            ...
            logger.error(
                f"clean_for_tvdb Title conversion error '{
                    title!r}': {
                    str(e)}")
            return ""
