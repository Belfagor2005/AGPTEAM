...
class AgpDownloadThread(Thread):
    ...
    def search_tmdb(
        ...
        def _strip_year_for_search(t):
            ...
            # remove trailing year: "Point break 2015" -> "Point break" but
            # keep "2012"
            ...
            if response.status_code == codes.ok:
                try:
                    ...
                    return self.downloadData2(
                        data, dwn_poster, shortdesc, fulldesc)
    ...
    def search_omdb(
        ...
        try:
            ...
            if data.get(
                    "Response") == "True" and url_poster and url_poster != "N/A":
                callInThread(self.savePoster, url_poster, dwn_poster)
                if exists(dwn_poster):
    ...
    def search_imdb(
        ...
        try:
            ...
            if url_poster and url_poster[0]:
                ...
                if exists(dwn_poster):
                    return (
                        True, f"[SUCCESS] IMDb poster match: {
                            match['title']} ({
                            match['year']})")
            ...
            for posterlst in plst:
                ...
                if url_poster_size and url_poster_size[0]:
                    ...
                    if self.title_safe == get_title:
                        ...
                        h_tar = int(h_tar)
                        url_poster = sub(
                            r'/\d+x\d+/', "/{}x{}/".format(w_tar, h_tar), url_poster)
    ...
    def search_molotov_google(
        ...
        try:
            ...
            if molotov_table[0]:
                return self.handle_poster_result(
                    molotov_table,
    ...
    def handle_poster_result(
        ...
        if len(pltt) > 0:
            ...
            callInThread(self.savePoster, url_poster, dwn_poster)
            if exists(dwn_poster):
                return True, f"[SUCCESS {platform}-google] Found poster for {
                    self.title_safe} => {url_poster}"
        else:
    ...
    def search_google(
        ...
        try:
            ...
            for pl in posterlst:
                ...
                callInThread(self.savePoster, url_poster, dwn_poster)
                if exists(dwn_poster):
                    return True, f"[SUCCESS google] Found poster for {
                        self.title_safe} => {url_poster}"

            return False, f"[SKIP : google] No poster found for {
                self.title_safe}"
        ...
        try:
            ...
            if response.status_code == codes.ok:
                results = response.text.replace(
                    ...
                    "")
                titles = findall(
                    '<li><a title="(.*?)" href="/en/work/(.*?)/"', results)
                ...
                for t, tid in titles:
                    ...
                    if t.lower() == self.title_safe.lower():
                        url_poster = "https://elcinema.com/en/work/{}/".format(
                            tid)
                        url_read = get(url_poster, headers=headers).text

                        img_match = findall(
                            '<img src="(https://.*?).jpg" alt=""', url_read)
                        if img_match:
                            poster = img_match[0] + ".jpg"
                            callInThread(self.savePoster, poster, dwn_poster)
                            if exists(dwn_poster):
                                return True, "[SUCCESS] Poster match: {}".format(
                                    t)
